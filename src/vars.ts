export const TOKEN = process.env.TOKEN || '';
export const BOT_OWNER = '289151449412141076';
export const DATABASE_TYPE = process.env.DATABASE || 'betadb';
export const DEFAULT_BANNED = process.env.DEFAULT_BANNED_WORDS || '';
export const DBLTOKEN = process.env.DBLTOKEN || '';
export const INVITE_URL =
  'https://discord.com/oauth2/authorize?client_id=773437651780894722&scope=bot&permissions=67497190';
export const CLIENT_ID = process.env.CLIENT_ID || '734968043813208085'; // Default is beta bot.
export const SUPPORT_URL = 'https://discord.gg/AJ58SKFBNf'; // Vivi support server.
export const VOTE_URL = `https://top.gg/bot/${CLIENT_ID}/vote`;
export const AVATAR_URL =
  'https://images.discordapp.net/avatars/773437651780894722/fd951f54c1e0d73fe69fd99d3e17ec78.png?size=256';
export const MONGODB = process.env.MONGODB || 'mongodb';
